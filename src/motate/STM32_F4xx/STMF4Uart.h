...
namespace Motate {
    ...
    struct _USARTHardware {
        ...
        static constexpr USART_TypeDef * const usart() {
...
//            return (uartPeripheralNumber == 0) ? USART0_IRQn : USART1_IRQn;
        ...
        DMA<USART_TypeDef *, uartPeripheralNumber> dma_ {_uartInterruptHandler};
        constexpr const DMA<USART_TypeDef *, uartPeripheralNumber> *dma() { return &dma_; };
        ...
        void setInterrupts(const uint16_t interrupts) {
            if (interrupts != UARTInterrupt::Off) {
                ...
                if (interrupts & UARTInterrupt::PriorityHighest) {
                	...
                	HAL_NVIC_SetPriority(usartIRQ(), 6, 0);
                }
                else if (interrupts & UARTInterrupt::PriorityMedium) {
                	HAL_NVIC_SetPriority(usartIRQ(), 9, 0);
                }
                else if (interrupts & UARTInterrupt::PriorityLow) {
                	HAL_NVIC_SetPriority(usartIRQ(), 12, 0);
                }
                else if (interrupts & kInterruptPriorityLowest) {
                	HAL_NVIC_SetPriority(usartIRQ(), 15, 0);
                }
        ...
        bool isConnected() {
            ...
            return (0);//usart()->US_CSR & US_CSR_CTS) == 0; // active LOW
        };


        // ***** Handle Tranfers
        bool startRXTransfer(char *buffer, const uint16_t length) {
            return 0;//dma()->startRXTransfer(buffer, length, true);
        };
    ...
    struct _UARTHardware {
        ...
        static constexpr USART_TypeDef * const uart() {
            switch (uartPeripheralNumber) {
                case (0): return USART3;
                case (1): return USART6;
        ...
        static constexpr IRQn_Type uartIRQ() {
            switch (uartPeripheralNumber) {
                case (0): return USART3_IRQn;
                case (1): return USART6_IRQn;
        ...
        int16_t readByte() {
            ...
            return (0);//uart()->UART_RHR & UART_RHR_RXCHR_Msk);
        };
